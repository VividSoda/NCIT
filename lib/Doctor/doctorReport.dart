...
class _DoctorReportState extends State<DoctorReport> {
  ...
  Future<List<PatientUploadedReport>> getReportInfo() async {
    ...
    final documents = FirebaseFirestore.instance.collection('doctors').doc(currentUser!.uid).collection('records');
    ...
    for (var doc in snapshot.docs) {
      ...
      Map<String, String>? patientDetails = await fetchPatientDetails(patientId);
                  ...
                  if(doctorUploadedReports.length>1 && index>0){
                    ...
                    return GestureDetector(
                      onTap: (){
                        Navigator.push(
                            context,
                          MaterialPageRoute(builder: (context) => PdfDoctorReports(doctorUploadedReport: doctorUploadedReports[index],))
                        );
                      },
                        child: PatientUploadedReportCard(
                            patientUploadedReport: doctorUploadedReports[index],
                            sameDate: sameDate
                        )
                    );
                  }
