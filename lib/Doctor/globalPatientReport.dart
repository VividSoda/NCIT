...
class GlobalPatientReport extends StatefulWidget {
  ...
  const GlobalPatientReport({Key? key, required this.patientId}) : super(key: key);
...
class _GlobalPatientReportState extends State<GlobalPatientReport> {
  ...
  //assign doctor uploaded reports for patient
  ...
  //Reports Uploaded by You
  //fetch doctor uploaded reports from firebase
  Future<List<PdfModel>> getDoctorUploadedReports() async{
    List<PdfModel> records = [];

    String patientId = widget.patientId;
    final document = FirebaseFirestore.instance.collection('doctors').doc(doctorId).collection('records').doc(patientId);
  ...
  // //fetch doctor name
  // Future<String> fetchDoctorDetails(String docId) async{
  //   String docName;
  //   final doctor = FirebaseFirestore.instance.collection('doctors').doc(docId);
  //   DocumentSnapshot<Map<String, dynamic>> snapshot = await doctor.get();
  //   Map<String, dynamic> data = snapshot.data()!;
  //   docName = data['name'];
  //   return docName;
  // }
  ...
  Widget build(BuildContext context) {
    ...
    bool recordExist = doctorUploadedRecordsExist || patientUploadedRecordsExist || reportsUploadedByOtherDoctorsExist;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          ) : recordExist? SingleChildScrollView(
            child: Column(
                    ...
                    if(doctorUploadedReports.length>1 && index>0){
                      ...
                      return GestureDetector(
                          onTap: (){
                            Navigator.push(
                                context,
                                MaterialPageRoute(builder: (context) => PdfViewer(pdfUrl: doctorUploadedReports[index].url))
                            );
                          },
                          child: PdfCard(
